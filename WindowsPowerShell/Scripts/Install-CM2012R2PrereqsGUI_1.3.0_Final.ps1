...
function Load-Form {
    ...
    $TabPageGeneral.Controls.AddRange(@($OutputBoxGeneral, $BlogLink, $LabelHeader, $LabelGeneralRestart, $LabelGeneralOS, $LabelGeneralPS, $LabelGeneralElevated, $GBGeneralReboot, $GBGeneralOS, $GBGeneralPS, $GBGeneralElevated, $GBGeneralValidation))
    $LabelGeneralRestart.Visible = $false
    $LabelGeneralOS.Visible = $false
    $LabelGeneralPS.Visible = $false
...
function Load-CAS {
    if (-not(($TabPageCAS.Controls | Measure-Object).Count -ge 1)) {
	    ...
	    $TabPageCAS.Controls.AddRange(@($ProgressBarCAS, $OutputBoxCAS, $CBCASDownloadPrereqs, $ButtonInstallCAS))
    }
...
function Load-Primary {
    if (-not(($TabPagePrimary.Controls | Measure-Object).Count -ge 1)) {
	    ...
	    $TabPagePrimary.Controls.AddRange(@($ProgressBarPrimary, $OutputBoxPrimary, $CBPrimaryDownloadPrereqs, $ButtonInstallPrimarySite))
    }
...
function Load-Secondary {
    if (-not(($TabPageSecondary.Controls | Measure-Object).Count -ge 1)) {
        ...
        $TabPageSecondary.Controls.AddRange(@($ProgressBarSecondary, $OutputBoxSecondary, $ButtonInstallSecondarySite))
    }
...
function Load-SiteSystemRoles {
    if (-not(($TabPageSiteRoles.Controls | Measure-Object).Count -ge 1)) {
        ...
        $TabPageSiteRoles.Controls.AddRange(@($ProgressBarSiteRoles, $OutputBoxSiteRoles, $RadioButtonSiteRoleMP, $RadioButtonSiteRoleDP, $RadioButtonSiteRoleAppCat, $RadioButtonSiteRoleEP, $ButtonInstallSiteRoles, $GBSiteSystemRoles))
    }
}
...
function Load-Other {
    if (-not(($TabPageOther.Controls | Measure-Object).Count -ge 1)) {
        ...
        $TabPageOther.Controls.AddRange(@($ProgressBarOther, $OutputBoxOther, $ButtonInstallOther, $RadioButtonOtherEAD, $RadioButtonOtherWSUS, $RadioButtonOtherADK, $RadioButtonOtherSMC, $GBOther))
    }
}
...
function Interactive-SiteRolesControlVisibility {
    ...
    Begin {
        ...
        $ControlsArrayList.AddRange(@("Option Enrollment Point","Option Management Point","Option Distribution Point","Option Application Catalog","Option Site System Roles"))
        switch ($Mode) {
...
function Interactive-OtherControlVisibility {
    ...
    $Mode,
    [parameter(Mandatory=$true)]
    [ValidateSet("EAD","WSUS","ADK","SMC")]
    $Module
    )
    Begin {
        $OtherControlsArrayList = New-Object System.Collections.ArrayList
        $OtherControlsArrayList.AddRange(@("Option Extend Active Directory","Option Install WSUS","Option Install Windows ADK","Option System Management Container","Option Make a selection"))
        switch ($Mode) {
...
function Validate-RunChecks {
    ...
    if ($ValidateCounter -ge 4) {
        ...
        Write-OutputBox -OutputBoxMessage "All validation checks did not pass successfully, remediate the errors and re-launch the tool" -Type "ERROR: " -Object General
    }
}
...
function Validate-Elevated {
    ...
    if ($UserWP.IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)) {
        $LabelGeneralElevated.ForeColor = "Green"
        $LabelGeneralElevated.Text = "Sufficient rights detected"
        ...
        $LabelGeneralElevated.ForeColor = "Red"
        $LabelGeneralElevated.Text = "Insufficient rights detected"
		$LabelGeneralElevated.Visible = $true
...
function Validate-RebootPendingCheck {
	...
	if ($ValidateRebootPending) {
        $LabelGeneralRestart.ForeColor = "Red"
        $LabelGeneralRestart.Text = "A reboot is pending"
        ...
        $LabelGeneralRestart.ForeColor = "Green"
        $LabelGeneralRestart.Text = "No reboot pending"
		$LabelGeneralRestart.Visible = $true
...
function Validate-OSCheck {
	...
	if (($OSProductType -eq 3) -and ($OSBuildNumber -ge 9200)) {
        ...
        $LabelGeneralOS.ForeColor = "Green"
        $LabelGeneralOS.Text = "Supported platform detected"
        ...
        if ($OSBuildNumber -lt 9200) {
            ...
            $LabelGeneralOS.ForeColor = "Red"
            $LabelGeneralOS.Text = "Unsupported platform detected"
            $LabelGeneralOS.Visible = $true
            return $false
        }
...
function Validate-PSCheck {
    if ($host.Version -ge "3.0") {
        ...
        $LabelGeneralPS.ForeColor = "Green"
        $LabelGeneralPS.Text = "Supported version detected"
        ...
        $LabelGeneralPS.ForeColor = "Red"
        $LabelGeneralPS.Text = "Unsupported version detected"
        $LabelGeneralPS.Visible = $true
        return $false
    }
}
...
function Install-CAS {
    ...
    if ($CBCASDownloadPrereqs.Checked -eq $true) {
        ...
        Write-OutputBox -OutputBoxMessage "Starting to verify that all features where successfully installed" -Type "INFO: " -Object CAS
        $ErrorVerify = 0
        $ProgressBarCAS.Value = 0
        $WinFeatures | ForEach-Object {
            $CurrentFeature = $_
...
function Install-Primary {
    ...
    if ($CBPrimaryDownloadPrereqs.Checked -eq $true) {
        ...
        Write-OutputBox -OutputBoxMessage "Starting to verify that all features where successfully installed" -Type "INFO: " -Object Primary
        $ErrorVerify = 0
        $ProgressBarPrimary.Value = 0
        $WinFeatures | ForEach-Object {
            $CurrentFeature = $_
...
function Install-Secondary {
    ...
    Write-OutputBox -OutputBoxMessage "Starting to verify that all features where successfully installed"  -Type "INFO: " -Object Secondary
    $ErrorVerify = 0
    $ProgressBarSecondary.Value = 0
    $WinFeatures | ForEach-Object {
        $CurrentFeature = $_
...
function Install-AppCat {
    ...
    Write-OutputBox -OutputBoxMessage "Starting to verify that all features where successfully installed" -Type "INFO: " -Object SiteRoles
    $ErrorVerify = 0
    $ProgressBarSiteRoles.Value = 0
    $WinFeatures | ForEach-Object {
        $CurrentFeature = $_
...
function Configure-LocalAdmin {
    ...
    $Group = [ADSI]("WinNT://$($LocalComputer)/$($LocalGroup)")
...
$Form.Text = "ConfigMgr 2012 R2 Prerequisites Installation Tool - 1.3.0"
...
$ButtonInstallSiteRoles.Add_Click({
    ...
    elseif ($RadioButtonSiteRoleEP.Checked -eq $true) {
        if (($ComboBoxSiteRolesEP.Text -eq "Enrollment Point") -or ($ComboBoxSiteRolesEP -eq "Enrollment Proxy Point")) {
...
$LabelGeneralRestart.Location = New-Object System.Drawing.Size(38,57)
...
$LabelGeneralOS.Size = New-Object System.Drawing.Size(170,15)
$LabelGeneralOS.Location = New-Object System.Drawing.Size(38,97)
...
$LabelGeneralPS.Size = New-Object System.Drawing.Size(170,15)
$LabelGeneralPS.Location = New-Object System.Drawing.Size(38,137)
...
$LabelGeneralElevated.Size = New-Object System.Drawing.Size(170,15)
$LabelGeneralElevated.Location = New-Object System.Drawing.Size(38,177)
...
$RadioButtonSiteRoleMP.Add_Click({
    ...
    Interactive-SiteRolesControlVisibility -Mode Disable
})
...
$RadioButtonSiteRoleEP.Add_Click({
    ...
    Interactive-SiteRolesControlVisibility -Mode Disable
    $TabPageSiteRoles.Controls.AddRange(@($ComboBoxSiteRolesEP, $GBEP))
    Interactive-SiteRolesControlVisibility -Mode Enable
})
...
$GBEP.Name = "Select a site system role"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUYmDL2YAtniGocVskASutAtn9
# Lt+gggpaMIIFIjCCBAqgAwIBAgIQAupQIxjzGlMFoE+9rHncOTANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMXFWXmVaKuGkit9
# 2abZIigAZXbxMA0GCSqGSIb3DQEBAQUABIIBAADnTmlVJuJ1MzKpFAwwKaN3UUy/
# EJhf8KKHtwI8PH16BBDqShMV1mTfuGNVYeiw4jjv0w7NlzHMwbYqh+8v38UbDrdF
# h6+DjlO52aQ86HKIt4q5276b8OTHtK1tw/M72K9uW+k377yZMvtEzAnXJJ+XjFbD
# gVznZJk4gA/fjtSAgQBBI6VfFeQFU+o+EFFrgin2KOH3C9iXVqAr/yoNCcSoAFe4
# YJ13WQmqqmv8EPHN29+7tKxFTTMYavzHoZqQZwWPi6BjAFaHcmtND4MJku029PR6
# Fmw0+Ov9l0Y1t2wr8vJZ2Zf3BZIotgDfYghtwaeZNu1d5M4lPBvR6nG0p8c=
