...
function Get-MemoryStatus {
  ...
  $MEMORYSTATUSEX = ($asm = [Object].Assembly).GetType("$(
    ...
    )$(Get-Variable type -ValueOnly)+MEMORYSTATUSEX"
  ).GetConstructor(
    ($bf = [Reflection.BindingFlags]36), $null, [Type[]]@(), $null
  ).Invoke($null)
  
  if ($asm.GetType($type).GetMethod(
    'GlobalMemoryStatusEx', [Reflection.BindingFlags]40
  ).Invoke($null, @($MEMORYSTATUSEX))) {
    $MEMORYSTATUSEX.GetType().GetFields($bf) | % {$ht = @{}}{
      $name = $_.Name -replace '\A\w', [Char]::ToUpper($_.Name[0])
      if ($_.Name -match '(?:(total)|(avail))') {
        $ht[$name] = '{0:f3} Gb' -f ($_.GetValue($MEMORYSTATUSEX) / 1Gb)
      }
      elseif ($_.Name -match 'load') {
        $ht[$name] = '{0} %' -f $_.GetValue($MEMORYSTATUSEX)
      }
    }{ $ht }
  }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvBDdBupuYvlkm/DfZ/rDpFtM
# miKgggpaMIIFIjCCBAqgAwIBAgIQBg4i3l65iHFvsYhyMrxXATANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFA3Z0aeGolrhd4xF
# Akn4GVAc4ayuMA0GCSqGSIb3DQEBAQUABIIBABvgKSNn7db8O3fo2iMu6mgAT4bQ
# rvJv5fTvmLW7Xm7l5DTb6+/PUOa4wPvfbO4iaM4wMwLtJKd9ETxC+WdCD5002VJ9
# yoMGIT0YfgM8u96kGEEx1As2R190MQVFzZPPhWy4heNzxHnTSyJUY41PVXhlozzy
# a2rdItM+YJ3De7Eqd3iOPVjVG2siAr9XIv2pk7e8cZcXucO5cnNDxrdrs32JXiBv
# EEIUXFlUu2iWOTu9Zb+l5fH/h1NqUqSlCTZYd0OruDrUPscPs1K55nBx9WQA0dJ4
# DBnqgv8kUrwPrn6ViSwqJZzMbibWBA9k2UhKfOHoTC6Glthn5Vv95Fqxilc=
