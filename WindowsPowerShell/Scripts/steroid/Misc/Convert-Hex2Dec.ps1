...
if (!(Test-Path alias:hex2dec)) {Set-Alias hex2dec Convert-Hex2Dec}
...
function Convert-Hex2Dec {
  ...
  $Numbers | % {[Int32]$n = $null}{
    if ([Int32]::TryParse($_, [ref]$n)) {
      if (!$_.StartsWith('0')) { '{0} = 0x{1:X}' -f $_, $n }
      else { 'Invalid dec: {0}' -f $_ }
    }
    else {
      try {
        if ($_.StartsWith('x', [StringComparison]::OrdinalIgnoreCase)) { $_ = '0' + $_ }
        '{0} = {1}' -f $_, ([Convert]::ToInt32($_, 16))
      }
      catch [FormatException]   { 'Invalid hex: {0}' -f $_ }
      catch [Management.Automation.MethodInvocationException] {
        'Out of range.'
      }
      catch [OverflowException] { $_.Exception }
    }
  } #foreach
}

Export-ModuleMember -Alias hex2dec -Function Convert-Hex2Dec

# SIG # Begin signature block
# MIINGAYJKoZIhvcNAQcCoIINCTCCDQUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUUXtUJ9L+p4QQrm79IWL9g57U
# 04WgggpaMIIFIjCCBAqgAwIBAgIQBg4i3l65iHFvsYhyMrxXATANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMZUi5ZsqXm6X20L
# VtsunFzagghMMA0GCSqGSIb3DQEBAQUABIIBAGzTvOzccdcPXFfofsaPall6VjOr
# N0oy7qJhABboqtAFkZ3NOlbfZSkqQDqmFnw9bkY01TmmLS7++HNRqaFQJbqmMV8U
# YGyegEq/sz03mqbzSUc57axB70eThR5FYAZUYbXJx6E/3bLfVIKbnRwCGhU5KH61
# AuUeUwLK+iY7Zuo+r04E3McmCDbrimAw2QVFjE2HRyFRlAL5VRRKg779SwKnqpmL
# EYwOp7hRfB5UvIUwnpfJJMpHcMoqmkETHuDjDgKW+kpt2GpYhymOQOKwjqH92H/j
# eNxNZfHttzJVDYkZUya33wEkNt3rRdOa9PPP8FEg68HVRgKCtVUIbMpSoqU=
