Function Get-WUInstall
{
	...
	Process
	{
		...
		{	
			$UpdateAccess = $true
			Write-Progress -Activity "[1/$NumberOfStage] Post search updates" -Status "[$NumberOfUpdate/$PreFoundUpdatesToDownload] $($Update.Title) $size" -PercentComplete ([int]($NumberOfUpdate/$PreFoundUpdatesToDownload * 100))
			...
			If($Category -ne "")
			{
				...
				Foreach($Cat in $Category)
				{
					...
					{
						$UpdateAccess = $false
					} #End If !($UpdateCategories -match $Category)
					Else
					{
						$UpdateAccess = $true
						Break
					} #End Else !($UpdateCategories -match $Category)
			...
			If($KBArticleID -ne $null)
			{
				...
				If(!($KBArticleID -match $Update.KBArticleIDs))
				{
					$UpdateAccess = $false
				} #End If !($KBArticleID -match $Update.KBArticleIDs)								
			} #End If $KBArticleID -ne $null
