...
function Get-ClockRes {
  ...
  begin {
    ...
    $href = New-Object Runtime.InteropServices.HandleRef(
      ...
      })[0].Invoke($null, 'ntdll.dll')
    )
    
    if (($ptr = $m[1].Invoke(
      $null, @([Runtime.InteropServices.HandleRef]$href, $func)
    )) -eq [IntPtr]::Zero) {
      'Could not locate NtQueryTimerResolution entry point.'
      return
    }
    ...
    Set-Content function:delegate {
      ...
      if (!(($mb = dynmod).GetTypes() | ? { $_.Name -eq $Delegate })) {
        $par  = @([Int32].MakeByRefType(), [Int32].MakeByRefType(), [InT32].MakeByRefType())
        $type = $mb.DefineType(
          $Delegate, 'AnsiClass, Class, Public, Sealed', [MulticastDelegate]
        )
        $ctor = $type.DefineConstructor(
          'HideBySig, Public, RTSpecialName', 'Standard', $par
        )
        $ctor.SetImplementationFlags('Managed, Runtime')
        $meth = $type.DefineMethod(
          'Invoke', 'HideBySig, NewSlot, Public, Virtual', [Int32], $par
        )
        $par | % {$i = 1}{
  ...
  process {
    [Int32]$max = $min = $cur = 0
    if ((delegate).Invoke([ref]$max, [ref]$min, [ref]$cur) -eq 0) {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUG/0xi0/lK7IstSJ5PAPPtP0A
# 7+igggpaMIIFIjCCBAqgAwIBAgIQBg4i3l65iHFvsYhyMrxXATANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFO7neA57mFJxVWgC
# 23IgxzFFCkIXMA0GCSqGSIb3DQEBAQUABIIBAEaY7IseYvXBRdB5jo/Bn56OPxFz
# euXyo33KLTLrts//mrLn4FU86sHgIvOyfXaO/y6Z2hUgh4X8uKyyeNRQV1lbBXcX
# 5vFF0h2s/y50woqPSgOC2oDryotCnh/LAt+Z4Qj14BIARoL2PgLT2pRaKyMwtgP6
# jVlrh8Z9uwgAlfxAUTnpjt4U7aF5vEL3Fjx5QtaO4G7qZj8AO6ed1f16bh34UPYd
# ETBRB/lZa7hiQf7ciqiaJF7+wLz7OF6xxHL9qXZqTWEgYcNn0Jgca3TcVbe7R2kh
# naxDvOrjKVta7C2q4riZsoZ/2/KBtU7H3spEsuJGrXalQOlWPbF9yJ0Wx28=
