...
function Test-Assembly {
  <#
    .SYNOPSIS
        Test an assembly that it was installed in GAC.
    .OUTPUTS
        String or Boolean if specified -BooleanOnly parameter.
    .NOTES
  ...
  Add-Type -AssemblyName ($asm = 'System.Deployment')
  
  try {
    $AssemblyInfo = ([AppDomain]::CurrentDomain.GetAssemblies() | ? {
      $_.ManifestModule.ScopeName.Equals("$($asm).dll")
    }).GetType(
      "$($asm).Application.Win32InterOp.SystemUtils"
    ).GetMethod(
      'QueryAssemblyInfo', [Reflection.BindingFlags]40
    ).Invoke($null, @(4, $AssemblyName)) #4 - QueryAssemblyInfoFlags.GetCurrentPath
    
    $res = $AssemblyInfo.GetType().GetField(
      'currentAssemblyPath', [Reflection.BindingFlags]36
    ).GetValue($AssemblyInfo)
  }
  catch {}
  finally {
    switch ($BooleanOnly) { $true{[Boolean]$res} $false{$res} }
  }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/avs84Ll2bPBWZsR7WVwxSBT
# vbSgggpaMIIFIjCCBAqgAwIBAgIQBg4i3l65iHFvsYhyMrxXATANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBhBRp2K3OrQ4Qpo
# pgpSO6jkfVqFMA0GCSqGSIb3DQEBAQUABIIBADGuSBbAqwSYUfPmgYrlyByWZRaI
# VfjAaGhNqxCj9ecXr3M1ayzEmZBcKsDz68jFvGFkFi1ytcQf+8MZc832lP3JYDsI
# 7F3J3lOKz0gKj4mvWFyP9XR7VCC3QSk+Vr40yRYfMSTC9iR/6vlwMA7Ns61z7EwY
# m/e5+jmOdNB86CjmuraglqZGrRd+sHBDE5pNb4DQe18Qdy2ngU1Jm7jnd6aDyPPo
# 9j+tywE4Zi1LHfLsTBqzSck5sURMt/OL1uAnm4Wpfr0zgqDJkb3BZ2tNd4Kn7q6u
# 8TDq0ByiobmYK12+eAb396rZisb1C9su9PRHR0VTkqz8YV8+FKeErZAmvtc=
