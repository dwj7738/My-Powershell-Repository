...
Function WMILookup {
    foreach ($StrComputer in $colComputers){
            write-host("Doing WMI Lookup for Computer $strComputer")
            write-host("Doing WMI Lookup for Computer System $strComputer")
            $GenItems1 = gwmi Win32_ComputerSystem -Comp $StrComputer
            $Genitems1
            write-host("Doing WMI Lookup for Operating System $strComputer")
            $GenItems2 = gwmi Win32_OperatingSystem -Comp $StrComputer
            $Genitems2
            write-host("Doing WMI Lookup for Bios $strComputer")
            $SysItems1 = gwmi Win32_BIOS -Comp $StrComputer
            $Sysitems1
            write-host("Doing WMI Lookup for Computer Timezone $strComputer")
            $SysItems2 = gwmi Win32_TimeZone -Comp $StrComputer
            $Sysitems2
            write-host("Doing WMI Lookup for WmiSetting $strComputer")
            $SysItems3 = gwmi Win32_WmiSetting -Comp $StrComputer
            $Sysitems3
            write-host("Doing WMI Lookup for Processor $strComputer")
            $ProcItems1 = gwmi Win32_Processor -Comp $StrComputer
            write-host("Doing WMI Lookup for Computer Memory $strComputer")
            $ProcItems1
            $MemItems1 = gwmi Win32_PhysicalMemory -Comp $StrComputer
            $MemItems1
            $memItems2 = gwmi Win32_PhysicalMemoryArray -Comp $StrComputer
            $MemItems2
            write-host("Doing WMI Lookup for Disks $strComputer")
            $DiskItems = gwmi Win32_LogicalDisk -Comp $StrComputer
            $DiskItems
            write-host("Doing WMI Lookup for Network Adapter $strComputer")
            $NetItems = gwmi Win32_NetworkAdapterConfiguration -Comp $StrComputer 
            $netitems
            write-host("Doing WMI Lookup for Software Features $strComputer")
            $SoftwareItems = gwmi Win32_SoftwareFeature -Comp $strComputer
            $softwareitems
            }
...
foreach ($objItem in $SoftwareItems)
    {
    if(obj.item.Attributes -eq "1") 
        {
        $Sheet7.Cells.Item($intRowNet, 1) = $StrComputer
        $Sheet7.Cells.Item($intRowNet, 2) = $objItem.Accesses
        $Sheet7.Cells.Item($intRowNet, 3) = $objItem.Attributes
        $Sheet7.Cells.Item($intRowNet, 4) = $objItem.Caption
        $Sheet7.Cells.Item($intRowNet, 5) = $objItem.Description
        $Sheet7.Cells.Item($intRowNet, 6) = $objItem.IdentifyingNumber
        $Sheet7.Cells.Item($intRowNet, 7) = $objItem.InstallDate
        $Sheet7.Cells.Item($intRowNet, 8) = $objItem.InstallState
        $Sheet7.Cells.Item($intRowNet, 9) = $objItem.LastUse
        $Sheet7.Cells.Item($intRowNet, 10) = $objItem.Name
        $Sheet7.Cells.Item($intRowNet, 11) = $objItem.ProductName
        $Sheet7.Cells.Item($intRowNet, 12) = $objItem.Status
        $Sheet7.Cells.Item($intRowNet, 13) = $objItem.Vendor
        $Sheet7.Cells.Item($intRowNet, 14) = $objItem.Version
        $intRowNet = $intRowNet + 1
        }
    }
$intRow = $intRow + 1
...
foreach ($StrComputer in $colComputers){
            ...
            $SoftwareItems = gwmi Win32_SoftwareFeature -comp $strComputer -Credential $cred}
...
Function ManualEntry {
      ...
      if( !(Test-Connection $colComputers) ){
...
if ($colComputers.count -ge 1)
{
    ...
    #Create Heading for Software
    $Sheet7.Cells.Item(1,1) = "Device_Name"
    $Sheet7.Cells.Item(1,2) = "Accesses"
    $Sheet7.Cells.Item(1,3) = "Attributes"
    $Sheet7.Cells.Item(1,4) = "Caption"
    $Sheet7.Cells.Item(1,5) = "Description"
    $Sheet7.Cells.Item(1,6) = "IdentifyingNumber"
    $Sheet7.Cells.Item(1,7) = "InstallDate"
    $Sheet7.Cells.Item(1,8) = "InstallState"
    $Sheet7.Cells.Item(1,9) = "LastUse"
    $Sheet7.Cells.Item(1,10) = "Name"
    $Sheet7.Cells.Item(1,11) = "ProductName"
    $Sheet7.Cells.Item(1,12) = "Status"
    $Sheet7.Cells.Item(1,13) = "Vendor"
    $Sheet7.Cells.Item(1,14) = "Version"
    ...
    Write-Output ("Nothing to find - Report Terminated") 
    }
# SIG # Begin signature block
# MIINGAYJKoZIhvcNAQcCoIINCTCCDQUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUNGMDBjyMGkjCuhify6Y7GEC4
# LQigggpaMIIFIjCCBAqgAwIBAgIQAupQIxjzGlMFoE+9rHncOTANBgkqhkiG9w0B
...
# NwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFIXt/PrjNZ+mekuV
# y2z1MQmmuPh6MA0GCSqGSIb3DQEBAQUABIIBAGwSOXeP0cLW6bmONd9//91qXFc5
# jM3PoHBrNLguOMpYGsrqpGLrrW/0cPOJawh7ccyJx4/t1g0foFmj2Ag9mEJbpW9j
# Ta/lIE56vn3pEPWGjiPkbGlMIBtRu8D0/UzeT3UI/VR0N7oKomwDTZnyI9iHFmfv
# LbV0We/tQQsf57SFyhveoPLdkTCptc9yulKb19Z6z1HSyD6tcucRiLaLe/DypikW
# bQa7cFRwcdccg99PPZsJglYie3nDW552xIGWcyYGCkL91iJReYvvq+e+/hJlVdfY
# BcaR3czvtiOJ10ntKDr9tqzNJ/NB8tS3V/soiFKCCsdP+ehC6RugcMJbqxU=
